...
NeuralNet.prototype.next = function (candle) {
  // learn from data on the fly
  this.predict();
  this.net.propogate(0.2, [candle]);
  // add the candle to the existing setup
...
NeuralNet.prototype.train = function (data, opts) {
  ...
  opts = opts || {};
  opts.error = opts.error || 0.000001;
...
NeuralNet.prototype.train_from_stream = function (stream, opts) {
  var self = this;
  var data = [];
  return new Promise(function (resolve, reject) {
    ...
    stream.on('end', function () {
      resolve(self.train_from_array(data, opts));
    });
  });
};
